...
void main(void)
{
    ...
    /* Se declaran diferentes variables y apuntadores de la estructura empleado para que el lector pueda apreciar también las diferentes formas en que los campos reciben valores. */

    /* En el programa principal se leen los campos de una variable, e3, y un apuntador de tipo estructura, *e1. */
    e1 = (struct empleado *)malloc(sizeof(struct empleado)); // Reemplaza 'new' con 'malloc'
    printf("\nIngrese el nombre del empleado 1: ");
    scanf("%s", (*e1).nombre); // Cambia '&' por '*'
    fflush(stdin);
    printf("Ingrese el departamento de la empresa: ");
...
e2 = (struct empleado *)malloc(sizeof(struct empleado)); // Reemplaza 'new' con 'malloc'
Lectura(e2);
