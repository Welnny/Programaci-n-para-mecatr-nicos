...
if (isalpha (p1))
    if (isupper(p1))
        ...
        /* La función tolower convierte de mayúscula a minúscula. Si laletra es minúscula no la modifica. La función toupper, por otra parte, convierte de minúscula a mayúscula. Si la letra es mayúscula no la modifica. */
    else
        printf("%c es una letra minúscula \n", p1);
else
    printf("%c no es una letra \n", p1);
